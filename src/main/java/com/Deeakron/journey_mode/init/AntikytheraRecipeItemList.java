...
public class AntikytheraRecipeItemList {
    ...
    public AntikytheraRecipeItemList(MinecraftServer server) {
        ...
        for (int i = 0; i < recipeLocations.length; i++) {
            ...
            for (int j =0; j < 9; j++) {
                ...
                try {
                    if (manager.byKey(recipeLocations[i]).get().getIngredients().get(j) != null) {
                        tempItems2[j] = ForgeRegistries.ITEMS.getKey(manager.byKey(recipeLocations[i]).get().getIngredients().get(j).getItems()[0].getItem());
                    } else {
            ...
            tempItems2[9] = ForgeRegistries.ITEMS.getKey(manager.byKey(recipeLocations[i]).get().getResultItem().getItem());
